...
const DropLabel = ({ item }: { item: DomainDropSet["items"][0] }) => {
  ...
  const domains = useMemo(() => {
    const drops = DomainDropSets.filter((d) =>
      [...d.items, ...(d.itemsAux || [])].includes(item)
    );
    ...
    return (
      ...
      Domains.filter((d) => d.drops.some((d) => drops.includes(d)))
        ...
        .sort((a, b) => {
          const category =
            DomainCategories.indexOf(a[1]) - DomainCategories.indexOf(b[1]);
        ...
        return (
          <HStack
            ...
            title={
              auxiliary
                ? formatMessage({
                    defaultMessage: "Auxiliary drop based on probability",
                  })
                : undefined
            }
          >
            ...
            <div>
              ...
              <FormattedMessage
                ...
                values={{
                  domain: (
                    <Link
                      color={`${category.colorHint}.${useColorModeValue(
                        "500",
                        "300"
                      )}`}
                      fontWeight="semibold"

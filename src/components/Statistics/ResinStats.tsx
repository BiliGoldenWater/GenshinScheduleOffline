...
const TotalPanel = () => {
  ...
  const value = stats.reduce(
    (total, { resinsSpent }) => total + resinsSpent,
    0
  );
    ...
    <Stat flex={1} textAlign="center">
      ...
      <StatHelpText>
        <FormattedMessage
          ...
          values={{
            percent: Math.round((value / (ResinsPerDay * retention)) * 100),
          }}
        />
      </StatHelpText>
    </Stat>
...
const PeakPanel = () => {
  ...
  return (
    <Stat flex={1} textAlign="center">
      ...
      <StatHelpText>
        {peak ? (
          DateTime.fromMillis(peak.time).toLocaleString()
        ) : (

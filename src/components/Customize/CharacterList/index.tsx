...
const CharacterList = () => {
  ...
  const all = useMemo(
    () =>
      CharacterSearch.search("").sort((a, b) => a.name.localeCompare(b.name)),
    []
  );
  const results = useMemo(
    () => new Set(CharacterSearch.search(search)),
    [search]
  );
  ...
  return (
    <VStack
      align="stretch"
      spacing={4}
      display={results.size ? undefined : "none"}
    >
      <Heading size="md">
      ...
      <Grid gap={2} className={styles.grid}>
        {all.map((character) => (
          <chakra.div
            key={character.name}
            display={results.has(character) ? undefined : "none"}

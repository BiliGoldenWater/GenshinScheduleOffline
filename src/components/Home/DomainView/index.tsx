...
const DomainView = () => {
  ...
  let domains = useMemo(() => {
    ...
    for (const drops of DomainDropSets.filter((drops) =>
      drops.days.includes(today)
    )) {
      ...
      const addMaterial = (
        ...
        if (
          ...
          if (domain) {
            ...
            const group = scheduled.materials.find(
      ...
      const addArtifact = (
        linked: ScheduledDomain["artifacts"][0]["linked"]
      ) => {
        if (
          [
            ...drops.items,
    ...
    results = results.filter(
      ({ region }) =>
        filterRegion === "all" || region.name.toLowerCase() === filterRegion
    );

    for (const domain of results) {
      domain.materials.sort((a, b) =>
      ...
      domain.artifacts.sort((a, b) =>
        a.linked.name.localeCompare(b.linked.name)
      );
    }

    results = results.sort((a, b) => {
      const category =
        DomainCategories.indexOf(a.category) -
        DomainCategories.indexOf(b.category);
    ...
    if (filter === "efficiency") {
        ...
        for (const domain of results) {
          for (const group of domain.materials) {
            group.auxiliary === auxiliaryFilter &&
              (group.linked = group.linked.filter(({ name }) =>
                add(`${group.material.name}|${name}`)
              ));
          }

          domain.materials = domain.materials.filter(
            ({ linked }) => linked.length
          );
          domain.artifacts = domain.artifacts.filter(
            ({ linked: { name }, auxiliary }) =>
              auxiliary !== auxiliaryFilter || add(name)
          );
        }
      };
      ...
      results = results
        .filter(
          ({ materials, artifacts }) => materials.length || artifacts.length
        )
        .map((domain) => {
          let score = 0;
          ...
          for (const { linked, drops } of domain.materials) {
            score += linked.length;
            highlighted += linked.filter(({ name }) =>
  ...
  const domainColumns = useMemo(() => {
    ...
    const addCategories = (...categories: DomainCategory[]) => {
      results.push(
        domains.filter(({ category }) => categories.includes(category))
      );
    };
  ...
  return (
    ...
    >
      {domains.length ? (
        <Grid
          className={domainColumns.length > 1 ? styles.grid : undefined}
          gap={4}
        >

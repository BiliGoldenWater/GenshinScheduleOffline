...
export function getNextRefreshTime(
  ...
  const utc = current.toUTC();
  let local = DateTime.utc(utc.year, utc.month, utc.day, utc.hour).setZone(
    current.zone
  );
  ...
  const nextWeek =
    current.weekday > rWeekday ||
    (current.weekday === rWeekday && current.hour >= ServerResetHour);

  return local
    .plus({ week: nextWeek ? 1 : 0 })
    .set({ weekday: rWeekday, hour: ServerResetHour });
}
...
export function useFormatTime(
  time: DateTime,
  units: Exclude<TimeUnit, "year" | "week">[]
) {
  const parts: string[] = [];
  ...
  switch (name) {
    ...
    case "duration.millisecond":
      return formatMessage(
        {
          defaultMessage:
            "{value, plural, one {# millisecond} other {# milliseconds}}",
        },
        { value }
      );

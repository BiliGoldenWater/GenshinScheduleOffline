  ...
  return useMemo(() => {
    return tasks
      .filter((task) => {
        ...
        return (
          (showHidden && !visible) || (showDone && !due) || (visible && due)
        );
      })
      .sort((a, b) => {
...
export function useTaskCreator() {
  ...
  return useCallback(
    async (
      ...
      openMap && (window.location.pathname = "/home/map");
    },
    [center, setTask, synchronize]
  );
}
  ...
  return useCallback(
    (done: boolean) => {
      if (done) {
        ...
        setStats(
          (stats) =>
            stats && { ...stats, tasksDone: Math.max(0, stats.tasksDone + 1) }
        );
      } else {
        setTask((task) => ({
          ...task,
